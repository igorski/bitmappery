...
describe( "rotate Layer action", () => {
    const layer = LayerFactory.create({
        transform: TransformFactory.create({ rotation: 2 }),
    });
    ...
    it( "should be able to set the rotation of a specific Layer within the Document", () => {
        ...
        expect( store.commit ).toHaveBeenCalledWith( "updateLayerTransform", {
            index: 2,
    ...
    it( "should restore the original rotation when calling undo in state history", () => {
        ...
        expect( store.commit ).toHaveBeenNthCalledWith( 2, "updateLayerTransform", {
            index: 2,
    ...
    it( "should reapply the new rotation when calling redo in state history", () => {
        ...
        expect( store.commit ).toHaveBeenNthCalledWith( 3, "updateLayerTransform", {
            index: 2,
            transform: { rotation: 3 },
        });
    });
