...
describe( "commit layer effects and transforms action", () => {
    ...
    const layer    = LayerFactory.create({
        effects: EffectsFactory.create({ rotation: 5, scale: 2 }),
    ...
    it( "should restore the original values when calling undo in state history", async () => {
        const { effects, filters, width, height, left, top, maskX, maskY } = layer;
        ...
        expect( store.commit ).toHaveBeenNthCalledWith( 2, "updateLayer", {
            index: 0,
            opts: {
                effects, filters, source: expect.any( Object ), mask: expect.any( Object ), width, height, left, top, maskX, maskY
            },
            recreateRenderer: true,
        });
    });
